...
class ReservaController {
    ...
    public function guardar() {
        ...
        // ── NOTIFICACIÓN POR CORREO CON PDF ───────────────────────────────────
        // Obtener la reserva recién creada para tener su ID
        $reservaCreada = $model->obtenerUltima($_SESSION['user']['id']);

        if ($reservaCreada) {
            $pdfBytes = $this->generarPdfBytes($reservaCreada);
            enviarCorreoReserva($_SESSION['user'], $reservaCreada, $pdfBytes);
        }
        // ─────────────────────────────────────────────────────────────────────
    ...
    // ── PDF INDIVIDUAL (descarga desde el dashboard) ──────────────────────────
    public function pdfReserva() {
        ...
        $pdfBytes = $this->generarPdfBytes($r);
        header('Content-Type: application/pdf');
        header('Content-Disposition: inline; filename="Reserva_' . str_pad($r['id'], 5, '0', STR_PAD_LEFT) . '.pdf"');
        echo $pdfBytes;
        exit;
    }
    ...
    // ── GENERAR PDF EN MEMORIA (devuelve string de bytes) ────────────────────
    // Se usa tanto para el correo como para la descarga directa.
    private function generarPdfBytes(array $r): string {
        ...
        // 'S' devuelve el PDF como string en memoria (no lo envía al navegador)
        return $pdf->Output('S');
    }
